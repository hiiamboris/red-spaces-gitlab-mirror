...
events/handlers/scrollable: define-handlers [
	...
	on-up [space path event] [unless stop-drag [pass]]
	...
	on-time [space path event delay [percent!]] [					;-- press & hold way of scrolling
		...
		set [_: _: item: _: subitem:] drag-path
		scrollable-space/move-by/scale
			space
...
events/handlers/list-view: extend-handlers 'scrollable [
	on-down     [space path event] [if update? [space/roll-edge]]
	on-key-down [space path event] [if update? [space/roll-edge]]
	timer: [
...
events/handlers/table: define-handlers []
events/handlers/table/columns: extend-handlers 'list-view []



events/handlers/button: define-handlers [
	on-down [space path event] [
		space/pushed?: yes
		start-drag path
		update
...
;@@ define-handlers won't accept inner style definitions because it doesn't know it's name
;@@ TODO: fix this
; events/handlers/rotor: define-handlers [
; 	ring: []
; ]

events/handlers/rotor: define-handlers []
events/handlers/rotor/ring: define-handlers [
	on-down [space path event] [
		rotor: get path/-2
...
events/handlers/field: define-handlers [
